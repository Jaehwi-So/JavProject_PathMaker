...
public class MainGUI {
	...
	// 참,거짓값 저장되는 클래스
	GUI_IsCheck gui_ischeck = new GUI_IsCheck();
	...
	void create_new(Frame fr) {
		...
		make.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				gui_ischeck.setCheck(true); // GUI_IsCheck에 있는 값이 true가 됨
				...
				try {
					if(sequence == 0) {
						...
						if(machineD.getName_list().size() == 0) {
							...
							loc.setText("<" + machineD.getName_list().get(sequence)+" & " +location_name+"> 소요시간 입력");
							sequence++;
						}
					}
					...
					JOptionPane.showMessageDialog(frame, "시간을 입력해주세요!");
				}
	...
	void delete_location() {
		...
		make.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try { // 2020.04.24 ★★★추가사항★★★ 목록에 입력한 장소가 없을때 장소가 삭제할 장소가 없다는 메세지창 출력 
					...
					JOptionPane.showMessageDialog(frame,"삭제할 장소가 없습니다");
				}
	...
	public void run_main_gui() {	
		...
		m.main_label = new TextArea("",0,0, TextArea.SCROLLBARS_VERTICAL_ONLY);
		m.main_label.setBounds(200, 170, 600, 320);
		m.main_label.setBackground(Color.white);
		...
		m.nowList.setBackground(Color.white);
		...
		load.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				m.machineD.load();
				m.gui_ischeck.setCheck(true);//2020.04.23 ★★★ 추가사항 ★★★ 
