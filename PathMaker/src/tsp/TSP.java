...
public class TSP {
	...
	int adj_mat[][];	//인접 행렬
	int lt_mat[]; //소요 시간
	int at_mat[];	//도착 시간 배열(정해지지 않았다면 -1). GUI상에서 미구현
	int at_type[]; //도착 타입(정해지지 않음, 시간 맞춰서 업무 시작, 시간 전에만 도착). GUI상에서 미구현
		...
		if (best) {
			...
			for (int i = 0; i <= n; i++) {
				if (i == n) {
					...
					s += "#" + name[u.path[i]]+" : [도착 :" + u.arrive[i] + "분 /STAY : " + lt_mat[u.path[i]] + "분 /출발 : " + u.gone[i] + "분]";
				}
				else {
					//System.out.printf("%s(%d) : [도착 : %d 시간소요 %d 출발 %d] ", name[u.path[i]], u.path[i], u.arrive[i], lt_mat[u.path[i]], u.gone[i]);
					s += "#" + name[u.path[i]]+" : [도착 :" + u.arrive[i] + "분 /STAY : " + lt_mat[u.path[i]] + "분 /출발 : " + u.gone[i] + "분]";
					//System.out.printf("-(%d)->", adj_mat[u.path[i]][u.path[0]]);
					s += "\n\t-( 이동시간 :" + adj_mat[u.path[i]][u.path[i+1]] + "분)->";
				}
				//System.out.println();
				s += "\n";
			}
			...
			s += "경과 시간 : " + u.time/60+"시간"+u.time%60+"분";
			s += "\n";
			...
			Frame f = new Frame("결과입니다.");
			Font font = new Font("", Font.BOLD, 20);
			f.setLayout(null);
			f.setBounds(100, 100, 800, 800);
			f.setBackground(Color.lightGray);
			
			Label sub = new Label("==== 최적의 경로 탐색을 마쳤습니다 ====");
		...
		while (hp.data.size() != 0) {
			...
			if (v.i == n - 2 && min_value > v.bound) {
				...
				if(((idx = at_mat[u.path[n - 1]]) >= u.time || idx == -1) && path_promising(u) && min_time > u.time) {
					u.time += adj_mat[u.path[n - 1]][start]; //일단 땜빵
					u.arrive[n] = u.time;
					u.gone[n] = u.time;
					u.complete = true;
					u.i++;
